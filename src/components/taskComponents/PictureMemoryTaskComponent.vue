<script setup lang="ts">
  ...
  const showImage = ref(props.task?.items ? new Array(props.task.items.length).fill(false) : []);
  ...
  const clickAnswer = (index: number) => {
    if (showImage.value[index] || finished.value || !ready.value)
      return;
    if (pairing2nd.value === -1) {
      ...
      if (items.value[pairing2nd.value] !== items.value[index]) {
        ready.value = false;
        setTimeout(() => {
          showImage.value[pairing2nd.value] = false;
          showImage.value[index] = false;
          pairing2nd.value = -1;
          ready.value = true;
        }, 2000);
      } else {
        pairing2nd.value = -1;
        if (showImage.value.every(x => x))
          done();
      }
    }
  }
  ...
  <div class="task-container">
    ...
    <div class="items-container" :style="{'grid-template-columns': 'repeat(' + columns + ', 1fr)'}">
        ...
        @click="clickAnswer(index)">
          <img :src="getResourceUrl(item)" v-if="showImage[index]">
        </div>
    </div>
    <div class="submit">
      <div class="buttons">
